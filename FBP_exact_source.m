...
while time <= time_end+dt%(time_end+dt/2.)
    [A,b,t_gmls,n_gmls,center_x,center_y,p1_prime] = velocity(curve,N,told,nold);
    told = t_gmls';
    nold = -n_gmls';
    ...
    radius_record(tdt+1,1) = max(radius);
    %radius error
    % rerr = sqrt(1/N(rr)*sum((radius-1/sqrt(1/4-1/2*time)).^2));%\|e_V\|_{L^2}
    rerr = abs(max(radius)-1/sqrt(1/4-1/2*time));
    r_error(tdt+1,1)=rerr;

    %velocity error
    verr = sqrt(1/N(rr)*sum((d_curve-radius.^3./4).^2));
    v_error(tdt+1,1) = verr;
    ...
    % [A,b,t_gmls,n_gmls,center_x,center_y,p1_prime] = velocity(mid_curve,N,told,nold);
    % mid_vn = -A\b- p1_prime';
    % %vn+1
...
function [A,b,t_gmls,n_gmls,center_x,center_y,p1_prime] = velocity(curve,N,told,nold)
rightpt = zeros(N,1);
leftpt = zeros(N,1);
A = zeros(N,N);
b_sint = zeros(N,1);
...
% update the boundary condition for the system of p2 with exact pressure p1 for -\Delta p=r^2
p1 = -1/16.*(x.^2 + y.^2).^2;
grad_p1 = [-1/4.*(x.^2+y.^2).*x -1/4.*(x.^2+y.^2).*y];
