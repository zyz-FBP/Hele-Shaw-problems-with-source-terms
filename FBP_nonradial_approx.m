...
%% Function approx
%% Domain
R = 3;%radial direction
Nth = 256;%angular direction
%% Gauss-Legendre nodes and weights on [0,R]
Nr = 2000;   % Gaussian quadrature points
[xg, wg] = lgwt(Nr, 0, R);   % xg = quadrature nodes, wg = weights
r = xg;
wr = wg;
theta = linspace(0,2*pi,Nth+1);
theta(end) = [];
...
%% exact source term
f = rr.^2.*exp(0.5/R*rr.*cos(tt));
f_bar = sum(sum(f .* W)) * dtheta / (pi*R^2);
f = f - f_bar;
%% Storage
A_coeff = zeros(Nmax+1,Mmax);
B_coeff = zeros(Nmax+1,Mmax);
beta = zeros(Nmax+1,Mmax);

%% Compute Neumann zeros
for n = 0:Nmax
    for m = 1:Mmax
        beta(n+1,m) = bessel_derivative_fzero(n,m);
    end
end
...
for n = 0:Nmax
    ...
    for m = 1:Mmax
        bnm = beta(n+1,m);
        Jn = besselj(n, bnm*r/R);
        basis_c = Jn .* cosnt;
        basis_s = Jn .* sinnt;
        norm_c = sum(sum(basis_c.^2 .* W)) * dtheta;
        A_coeff(n+1,m) = sum(sum(f .* basis_c .* W)) * dtheta / norm_c;
        if n > 0
            norm_s = sum(sum(basis_s.^2 .* W)) * dtheta;
            B_coeff(n+1,m) = sum(sum(f .* basis_s .* W)) * dtheta / norm_s;
        end
    end
end
...
while time <= time_end+dt%(time_end+dt/2.)
    [A,b,t_gmls,n_gmls,center_x,center_y,p1_prime] = velocity(curve,N,told,nold,R,f_bar, beta,Nmax,Mmax,A_coeff,B_coeff);
    told = t_gmls';
    nold = -n_gmls';
...
function [A,b,t_gmls,n_gmls,center_x,center_y,p1_prime] = velocity(curve,N,told,nold,R,f_bar,beta,Nmax,Mmax,A_coeff,B_coeff)
rightpt = zeros(N,1);
leftpt = zeros(N,1);
A = zeros(N,N);
b_sint = zeros(N,1);
...
[p1_approx,grad_p1_approx] = func_approx(curve,R, beta,Nmax,Mmax,A_coeff,B_coeff);
p1 = p1_approx - f_bar/4*(x.^2+y.^2);
grad_p1 = grad_p1_approx + [-f_bar/2*x -f_bar/2*y];
...
r =2.0+ D1.*cos(D2.*theta);
x = r .* cos(theta);
y = r .* sin(theta);
end

% approximate p1 and gradient of p1 at boundary
function [p1_approx,grad_p1_approx] = func_approx(curve,R, beta,Nmax,Mmax,A_coeff,B_coeff)
%% approximation
curve_x = curve(:,1);
curve_y = curve(:,2);
curve_r = sqrt(curve_x.^2 + curve_y.^2);
curve_theta = atan2(curve_y,curve_x);
curve_theta(curve_theta<0)=curve_theta(curve_theta<0)+2*pi;
p1_approx = zeros(length(curve_r),1);
pr = zeros(length(curve_r),1);
ptheta = zeros(length(curve_r),1);

%% Reconstruct approximation 
for n = 0:Nmax
    cosnt = cos(n*curve_theta);
    sinnt = sin(n*curve_theta);
    for m = 1:Mmax
        bnm = beta(n+1,m);
        Jn = besselj(n, bnm*curve_r/R);
        Jnp = 0.5*(besselj(n-1,bnm*curve_r/R) - besselj(n+1,bnm*curve_r/R));
        
        p1_approx = p1_approx + A_coeff(n+1,m) * Jn .* cosnt ./ (bnm/R)^2;
        pr = pr + A_coeff(n+1,m) * (bnm/R) * Jnp .* cosnt ./ (bnm/R)^2;
        if n > 0
            p1_approx = p1_approx + B_coeff(n+1,m) * Jn .* sinnt ./ (bnm/R)^2;
            pr = pr + B_coeff(n+1,m) * (bnm/R) * Jnp .* sinnt ./ (bnm/R)^2;
            ptheta = ptheta + (-n*A_coeff(n+1,m)/(bnm/R)^2*sinnt + n*B_coeff(n+1,m)/(bnm/R)^2*cosnt).*Jn;
        end
    end
end

grad_p1_approx = [cos(curve_theta).*pr - sin(curve_theta).*ptheta./curve_r sin(curve_theta).*pr + cos(curve_theta).*ptheta./curve_r];
end

% zeros of J_n'(x)
function beta = bessel_derivative_fzero(n,m)
    Jp = @(x) 0.5*(besselj(n-1,x) - besselj(n+1,x));
    count = 0;
    xleft = 1e-8;
    step = pi/50;
    xright = xleft + step;
    while count < m
        if Jp(xleft)*Jp(xright) < 0
            count = count + 1;
            root = fzero(Jp,[xleft,xright]);
        end
        xleft = xright;
        xright = xright + step;
    end
